...
def fit_fusion(net,
    ...
    mse = torch.nn.MSELoss() #.type(dtype) 
    noise_energy = mse(img_noisy_var_one, img_clean_var)
    ...
    for i in range(num_iter):
        ...
        def closure():
            ...
            # training loss 
            if mask_var is not None:
                loss = mse( out * mask_var , img_noisy_var_one * mask_var )
            elif apply_f:
                loss = mse( apply_f(out) , img_noisy_var_one )
            else:
                loss1 = mse(out, img_noisy_var_one)
                loss2 = mse(out, img_noisy_var_two)
                loss = loss1 + loss2
